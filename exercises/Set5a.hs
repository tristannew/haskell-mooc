...
buyOneMore (MkShoppingEntry item price cnt) = MkShoppingEntry item price (cnt+1)
...
fromBin (I rest) = 2 * fromBin rest + 1

toBin :: Int -> Bin
toBin 0 = O End 
toBin x = hereToHelp x End

hereToHelp :: Int -> Bin -> Bin
hereToHelp x y = if x == fromBin y then y else hereToHelp x (inc y)
