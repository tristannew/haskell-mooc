...
-- incrementKey x xs = Map.toList $ Map.adjust (+1) x (Map.fromList xs)
incrementKey x [y] = if x == fst y then [(fst y, snd y+1)] else [y]
incrementKey x (y:ys) = if fst y == x 
    then (fst y, snd y + 1) : incrementKey x ys
    else y : incrementKey x ys
...
-- freqs xs = foldr addOne [] xs
--     where
--         map = Map.empty
--         addOne x = if Map.lookup x map /= Nothing then Map.alter (+1) x map else Map.insert x 1 map
...
--     Map.member from bank && Map.member to bank && amount >= 0 && Map.lookup from bank - Just amount >= 0
--     then Map.adjust (\x -> x-amount) from (Map.adjust (+amount) to bank)
--     else bank
...
maxIndex arr = hereToHelp (assocs arr)
    where hereToHelp [x] = fst x
          hereToHelp (x:y:xs) = if snd x > snd y then hereToHelp (x:xs) else hereToHelp (y:xs) 
-- maxIndex arr = index
