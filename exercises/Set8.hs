...
rectangle x0 y0 w h = Shape f
  where f (Coord x y) = inBounds x x0 w && inBounds y y0 h
        inBounds a b c = a < (b+c) && a >= b
------------------------------------------------------------------------------
...
union shpx shpy = Shape f
  where f (Coord x y) = contains shpx x y || contains shpy x y

cut :: Shape -> Shape -> Shape
cut shpfst shpsnd = Shape f
  where f (Coord x y) = contains shpfst x y && not (contains shpsnd x y)
------------------------------------------------------------------------------
...
paint (Picture p1) (Shape s1) (Picture b1) = Picture newPic
  where newPic coords = if s1 coords then p1 coords else b1 coords
...
instance Transform Fill where
  apply (Fill color) (Picture f1) = Picture newPic
    where newPic coords = color

data Zoom = Zoom Int
  deriving Show

instance Transform Zoom where
  apply (Zoom x) p1 = zoom x p1

data Flip = FlipX | FlipY | FlipXY
  deriving Show

instance Transform Flip where
  apply FlipX (Picture p1) = Picture newPic
    where newPic (Coord x y) = p1 (Coord (-x) y)
  apply FlipY (Picture p1) = Picture newPic
    where newPic (Coord x y) = p1 (Coord x (-y))
...
instance (Transform a, Transform b) => Transform (Chain a b) where
  apply (Chain x y) p1 = apply x (apply y p1)
------------------------------------------------------------------------------
...
instance Transform BlurMany where
  ...
  apply (BlurMany x) pic = apply (BlurMany (x-1)) (apply Blur pic)
------------------------------------------------------------------------------
